...
void function RTKArmoryTopWeapons_OnClick( rtk_panel panel )
{
	rtk_struct model = RTKDataModel_GetStruct( panel.GetBindingRootPath() )
	if( RTKStruct_HasProperty(model, "weaponId" ) )
	{
		string weaponId = RTKStruct_GetString( model, "weaponId" )
		ItemFlavor ornull weapon = GetWeaponItemFlavorByClass(weaponId )
		if( weapon != null )
		{
			expect ItemFlavor( weapon )
			ItemFlavor category = WeaponItemFlavor_GetCategory( weapon )
			SetTopLevelCustomizeContext( category )

			array<ItemFlavor> weaponsInCategory = GetWeaponsInCategory( category )
			int tabIndex = 0
			foreach( int index, ItemFlavor weaponInCategory in weaponsInCategory )
			{
				tabIndex = index

				if( weaponInCategory == weapon )
					break
			}

			CustomizeWeaponMenu_SetStartTab( tabIndex )
