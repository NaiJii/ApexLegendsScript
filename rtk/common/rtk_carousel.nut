...
void function RTKCarousel_SetTitleForNextPage( rtk_behavior self )
{
	...
	rtk_array pages = self.PropGetArray( "pages" )
	int pageCount = RTKArray_GetCount( pages )
	if ( p.nextPageIndex >= pageCount )
		return

	rtk_struct entry = RTKArray_GetStruct( pages, p.nextPageIndex )
...
void function RTKCarousel_RefreshActivePage( rtk_behavior self )
{
	...
	{
		rtk_struct entry = RTKArray_GetStruct( pages, i )
		rtk_panel page = RTKStruct_GetPanel( entry, "page" )
...
{
	rtk_array pages = self.PropGetArray( "pages" )
	rtk_struct inEntry = RTKArray_GetStruct( pages, inIndex )
	rtk_struct outEntry = RTKArray_GetStruct( pages, outIndex )
