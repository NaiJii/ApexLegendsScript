...
void function SetupNarrativeDebugDevMenu()
{
	...
	SetupDevMenu( "Anniversary Debug Menu", SetDevMenu_S16AnniversaryDebug )
...
void function SetDevMenu_S16AnniversaryDebug( var _ )
{
	ChangeToThisMenu( SetupS16AnniversaryDebug )
}

void function SetupS16AnniversaryDebug()
{
	if ( GetActiveLevel() == "mp_rr_tropic_island_mu1" )
	{
		SetupDevCommand("Trigger Ship Sequence", "script Anniversary_TropicsTriggerCeleberationPlaneAnims()")
		SetupDevCommand("Trigger Ship Sequence [Only #1]", "script thread Anniversary_TropicsPlaneAnimThread(0)")
		SetupDevCommand("Trigger Ship Sequence [Only #2]", "script thread Anniversary_TropicsPlaneAnimThread(1)")
		SetupDevCommand("Trigger Ship Sequence [Only #3]", "script thread Anniversary_TropicsPlaneAnimThread(2)")

		SetupDevCommand("Spawn Beachball at Crosshair", "script DEV_CreateBeachballAtCrosshair(GP())")
	}

	if ( GetActiveLevel() == "mp_rr_divided_moon" )
	{
		SetupDevCommand("Trigger Ship Sequence", "script Anniversary_DMTriggerCeleberationPlaneAnims()")
		SetupDevCommand("Trigger Ship Sequence [Only #1]", "script thread Anniversary_DMPlaneAnimThread(0)")
		SetupDevCommand("Trigger Ship Sequence [Only #2]", "script thread Anniversary_DMPlaneAnimThread(1)")
		SetupDevCommand("Trigger Ship Sequence [Only #3]", "script thread Anniversary_DMPlaneAnimThread(2)")
	}

	if ( GetActiveLevel() == "mp_rr_desertlands_mu4" )
	{
		SetupDevCommand("Trigger Ship Sequence", "script Anniversary_DesertlandsTriggerCeleberationPlaneAnims()")
		SetupDevCommand("Trigger Ship Sequence [Only #1]", "script thread Anniversary_DesertlandsPlaneAnimThread(0)")
		SetupDevCommand("Trigger Ship Sequence [Only #2]", "script thread Anniversary_DesertlandsPlaneAnimThread(1)")
		SetupDevCommand("Trigger Ship Sequence [Only #3]", "script thread Anniversary_DesertlandsPlaneAnimThread(2)")
	}

	SetupDevCommand("Enable Red and White Theme", "playlist survival_dev")
	SetupDevCommand("Enable Black and Gold Theme", "playlist survival_dev_blackgoldtheme")
	SetupDevCommand("Enable Beach Party", "playlist survival_dev_beachparty")
	SetupDevCommand("Enable Beach Party (Lobby)", "launchplaylist survival_dev_beachparty_lobby")
}
